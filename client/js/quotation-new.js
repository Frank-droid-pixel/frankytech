(function () {
  let businessId = null, currency = 'USD', builder = null;
  ...
  async function loadCustomers() {
    ...
    if (items.length === 0) {
      ...
      FrankyAuth.showAlert(alertEl, 'You need at least one customer first. ');
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      const { quotation } = await FrankyAuth.api(`/api/quotations?businessId=${businessId}`, { method: 'POST', body: JSON.stringify(payload) });
      window.location.href = `/quotation-view.html?id=${quotation.id}`;
    } catch (err) {
